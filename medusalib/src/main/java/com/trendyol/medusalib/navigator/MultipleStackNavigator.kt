...
open class MultipleStackNavigator(
    ...
    override fun start(fragment: Fragment, fragmentGroupName: String) {
        start(fragment, fragmentGroupName, transitionAnimationType, null)
    }

    override fun start(fragment: Fragment, transitionAnimation: TransitionAnimationType) {
        start(fragment, DEFAULT_GROUP_NAME, transitionAnimation, null)
    }

    override fun startWithAnimation(fragment: Fragment, customAnimations: Pair<Int, Int>) {
        start(fragment, DEFAULT_GROUP_NAME, transitionAnimationType, customAnimations)
    }

    override fun start(fragment: Fragment, fragmentGroupName: String, transitionAnimation: TransitionAnimationType?, customAnimations: Pair<Int, Int>?) {

        val createdTag = tagCreator.create(fragment)
        val currentTabIndex = fragmentStackState.getSelectedTabIndex()
        val fragmentData = FragmentData(fragment, createdTag, transitionAnimation, customAnimations)
        ...
        if (fragmentStackState.isSelectedTabEmpty()) {
            ...
            val rootFragmentData = FragmentData(rootFragment, rootFragmentTag, transitionAnimation, customAnimations)
