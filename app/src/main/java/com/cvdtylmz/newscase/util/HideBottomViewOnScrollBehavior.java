package com.cvdtylmz.newscase.util;
...
public class HideBottomViewOnScrollBehavior<V extends View> extends CoordinatorLayout.Behavior<V> {
    ...
    private AnimationOnChangeListener listener;

    public void setListener(AnimationOnChangeListener listener) {
    ...
    public void onNestedScroll(
        ...
        if (dyConsumed > 0) {
            listener.animationChanged(AnimState.SLIDE_DOWN);
            slideDown(child);
        } else if (dyConsumed < 0) {
            listener.animationChanged(AnimState.SLIDE_UP);
            slideUp(child);
        }
    }
